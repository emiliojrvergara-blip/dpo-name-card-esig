...
const NAV = [
  ...
  { id: 'social', label: 'Social & Links', icon: SocialIcon },
  { id: 'card', label: 'My Card & QR', icon: QRIcon },
]

// ── Nav icons ─────────────────────────────────────────────────────────────────
function ProfileIcon({ active }) {
  return (
    <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={active ? C.blue : C.textSecondary} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
// ── Single form row inside a section ─────────────────────────────────────────
    ...
    <form onSubmit={handleSave}>
        ...
        <div style={{ position: 'relative', marginBottom: 12 }}>
          ...
          <button
            ...
            style={{
              ...
              background: C.blue, border: '2.5px solid #f5f5f7',
      ...
      <Section label="Personal Info">
        {[
          ...
          <FormRow key={key} label={label} last={i === arr.length - 1}>
            <AppleInput value={form[key]} onChange={e => update(key, e.target.value)} type={type} />
          </FormRow>
        ))}
      </Section>
      ...
      <Section label="Company Info">
        {[
          ...
          { label: 'Office', val: emp.office },
          { label: 'Company', val: emp.company },
          { label: 'Country', val: emp.country },
          { label: 'Office Phone', val: emp.officePhone },
      ...
      <button
        ...
        style={{
          ...
          background: saved ? '#34C759' : C.blue,
...
function SocialTab({ emp, onSave }) {
  ...
  function removeBtn(i) { setCustomButtons(b => b.filter((_, idx) => idx !== i)) }
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 4, marginBottom: 8 }}>
          ...
          {customButtons.length < 2 && (
            <button type="button" onClick={addBtn} style={{ fontSize: 14, color: C.blue, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
              + Add
            </button>
          )}
        </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: C.textSecondary, marginBottom: 4 }}>
                      Button Label <span style={{ color: btn.title.length >= 18 ? '#ff3b30' : C.textTertiary }}>{btn.title.length}/20</span>
                    </div>
                    <input
                      style={{ width: '100%', border: 'none', outline: 'none', fontSize: 15, color: C.textPrimary, background: 'transparent', fontFamily: 'inherit' }}
                      value={btn.title}
                  ...
                  <button type="button" onClick={() => removeBtn(i)} style={{ width: 28, height: 28, borderRadius: '50%', background: '#ff3b30', border: 'none', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 }}>
                    ✕
                  </button>
                </div>
  ...
  async function shareNFC() {
    if ('NDEFReader' in window) {
      try {
        ...
        alert('Ready to tap! Hold near an NFC tag.')
      } catch {
        alert('NFC write failed. Requires Android Chrome over HTTPS.')
      }
    } else {
    ...
    <div style={{ paddingTop: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        {shareActions.map(({ label, color, onClick, icon }) => (
          ...
          >
            <div style={{ width: 36, height: 36, borderRadius: 10, background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background 0.3s' }}>
              {icon}
            </div>
      ...
      <a
        ...
        style={{
          ...
          borderRadius: 14, color: C.blue, fontSize: 16, fontWeight: 600,
...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Segoe UI', sans-serif" }}>
      ...
      {/* Sidebar */}
      <aside style={{
        width: 256, background: 'rgba(255,255,255,0.85)',
        backdropFilter: 'blur(20px)',
        borderRight: `1px solid ${C.border}`,
        display: 'flex', flexDirection: 'column',
        position: 'fixed', top: 0, left: sidebarOpen ? 0 : -256, bottom: 0,
        ...
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.divider}`, display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{
            ...
            {emp.photo
              ...
              : <svg width={20} height={20} viewBox="0 0 24 24" fill={C.blue} opacity={0.7}><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
            }
          </div>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: C.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{emp.cardName}</div>
            <div style={{ fontSize: 11, color: C.textTertiary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{emp.email}</div>
          </div>
        </div>
      ...
      <main style={{ flex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 17, fontWeight: 600, color: C.textPrimary, letterSpacing: '-0.02em' }}>
            {NAV.find(n => n.id === activeTab)?.label}
          </span>
          {/* Tab pills for desktop feel */}
          <div style={{ width: 32 }} />
        </div>

        {/* Page content */}
        <div style={{ flex: 1, padding: '0 16px', maxWidth: 540, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
